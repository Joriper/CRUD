		...
		try:
			...
			if len(connection_object):
				if not CrudApi.objects.filter(register_email_id=connection_object[0]['admin_email'],todo_id=todo_id).values():
					...
					data.save()
					rsp=Response({'status':'OK','message':'Todo Created','id':todo_id,'name':dataname},status=status.HTTP_201_CREATED)
					return rsp
	...
	if 'LoggedIN' in request.session:
		try:
			...
			if len(connection_object):
				...
				return Response({'status':'OK','message':'Updated','todo_id':todo_id_handler},status=status.HTTP_200_OK)
			return Response({'status':'Failed','message':'Only Admin User are Allowed'},status=status.HTTP_401_UNAUTHORIZED)
		except:
	...
	if 'LoggedIN' in request.session:
		try:
			...
			if len(connection_object):
				...
				return Response({'status':'OK','message':'Todo Deleted','id':todo_id_handler},status=status.HTTP_200_OK)
			return Response({'status':'Failed','message':'Only Admin User are Allowed'},status=status.HTTP_401_UNAUTHORIZED)
		except Exception as e:
